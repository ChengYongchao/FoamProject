...
- bash ./configure --with-target-bits=64 --with-boot-jdk=/Java/jdk-11.0.8/ --with-debug-level=slowdebug
...
- 合并分支， 分支A:需被合并的分支；分支B：主分支； 首先 保证本地分支是clean状态并拉取了最新的代码，分支B同样操作并切换到分支B，在分支B执行merge操作 如 git merge A 再执行git push
